...
// New thing (10 May 2025)
var album_code = document.querySelector("#column_container_right > div.section_main_info.section_outer > div > div.album_title > input").value
...
if (tracklist != undefined) {
    for (let i = 0; i < all_tracks.length; i++) {
        ...
        this["btn_wmr" + i] = document.createElement("button")
        this["btn_wmr" + i].innerText = '📝'
        this["btn_wmr" + i].onclick = function () {
            album_code_ = album_code.slice(0, -1) // Necessary so we can put the artist - song string within the brackets
            navigator.clipboard.writeText(`"${album_code_},${artist_name} - ${song_text}]" - x
Y: ${year_name}
G: ${pri_genres}
L: ${lang_name}`)
        }
        ...
        if (all_tracks[i].getElementsByClassName("rendered_text").length != 0) {
            this["song" + i][0].insertAdjacentElement("afterend", this["btn_wmr" + i])
            ...
            this["song" + i].insertAdjacentElement("afterend", this["btn_wmr" + i])
...
// For getting no. of minutes
const totalLengthElement = document.getElementsByClassName("tracklist_total")[0];

let minutes = 0;

if (totalLengthElement) {
    const totalLengthText = totalLengthElement.innerHTML;

    // Use a regular expression to extract the number of minutes
    const match = totalLengthText.match(/Total length: (\d+):\d+/);

    if (match) {
        minutes = parseInt(match[1], 10); // Convert the captured group to a number
    } else {
        console.log("Unable to extract minutes");
    }
} else {
    console.log("tracklist_total element not found");
}
...
function album_row() {
    ...
    a_row_nu = `-${c}${artist_name}${c}${album_name}${c}${year_name}${c}${release_type}${c}${c}${c}${pri_genres}${c}${c}${c}${c}${lang_name}${c}${minutes}${c}${c}${curr_date}`

    navigator.clipboard.writeText(a_row_nu)
}
...
//New and improved version (10 May 2025)
//Even more new and improved (16 Aug 2025)
if (tracklist != undefined) {
    let ld_entry = `Artist: ${artist_name}
Album: ${album_name}
Genre: ${pri_genres}
Year: ${year_name}
Added: 
...
function add_row(table_id, song_name) {
    ...
    var cell10 = row.insertCell(10); //Language
    ...
    var cell17 = row.insertCell(17); //Added
...
function table_clipboard() {
    ...
    the_row = `✓${c}${song}${c}${artist}${c}${c}${year}${c}${fgenres}${c}${c}${c}${c}${c}${language}${c}${c}${c}${c}${c}${c}${c}${the_date}`
